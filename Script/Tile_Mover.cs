...
public class TileMover : MonoBehaviour
{
        ...
        foreach (TileMoveCommand cmd in moveCommands)
        {
            ...
            if (cmd.isMerge)
            {
                ...
                if (newTilePrefab != null && cmd.mergeTarget != null)
                {
                    ...
                    if (tileIndex != -1) // Jeœli znaleziono poprawny indeks
                    {
                        // Uzyskujemy dostêp do CollectionKey poprzez CollectionManager
                        string savedTiles = PlayerPrefs.GetString(CollectionManager.CollectionKey, "100000000000000000");

                        if (savedTiles[tileIndex] == '0') // Jeœli kafelek jeszcze nie jest odblokowany
                        {
    ...
    IEnumerator AnimateTile(GameObject tile, List<Vector3> path, float segmentDuration, TileMoveCommand moveCommand, System.Action onComplete)
    {
        if (path.Count < 2)
        {
            // Jeœli kafelek ju¿ jest na miejscu, po³¹czymy go natychmiast
            if (moveCommand != null && moveCommand.isMerge)
            {
                ...
                if (targetTile != null)
                {
                    ...
                    // Animacja przesuniêcia o jedno pole
        ...
        // Jeœli kafelek siê ³¹czy, dodatkowa animacja na³o¿enia siê na inny kafelek
        if (moveCommand != null && moveCommand.isMerge)
        {
            GameObject targetTile = moveCommand.mergeTarget.currentTile;
            if (targetTile != null)
    ...
    private int GetTileIndex(int tileValue)
    {
        ...
        return -1; // Jeœli wartoœæ nie jest w tablicy, zwraca -1
    }
